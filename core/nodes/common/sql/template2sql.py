...
def get_sql_from_template(query_template, bind_params):
    if not bind_params:
...
def apply_sql_template(query_template, sql_parameters_resolved):
    '''
    Apply a substituting parameters (dict) and return
    the final SQL.
    '''
    tts = TemplateToSql(param_style='pyformat')
    query_template, bind_params = tts.prepare_query(query_template, sql_parameters_resolved)
    return get_sql_from_template(query_template, bind_params)